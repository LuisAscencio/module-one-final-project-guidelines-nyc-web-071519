...
class Rocket < ActiveRecord::Base
    ...
    def self.stats
        table = Terminal::Table.new :title => "Rockets Stats", :headings => ['Rocket', 'Cost', "Height", "Diameter", "Mass"] do |t|
            t << [
                Rocket.first.rocket_name,
                Rocket.first.cost, 
                "#{Rocket.first.height} ft.", 
                "#{Rocket.first.diameter} ft.", 
                "#{Rocket.first.mass} lb."
            ]
            t << :separator
            t.add_row [
                Rocket.second.rocket_name, 
                Rocket.second.cost, 
                "#{Rocket.second.height} ft.", 
                "#{Rocket.second.diameter} ft.", 
                "#{Rocket.second.mass} lb." 
            ]
            t.add_separator
            t.add_row [
                Rocket.third.rocket_name, 
                Rocket.third.cost, 
                "#{Rocket.third.height} ft.", 
                "#{Rocket.third.diameter} ft.", 
                "#{Rocket.third.mass} lb." 
            ]
            t.add_separator
            t.add_row [
                Rocket.fourth.rocket_name, 
                Rocket.fourth.cost, 
                "#{Rocket.fourth.height} ft.", 
                "#{Rocket.fourth.diameter} ft.", 
                "#{Rocket.fourth.mass} lb."
            ]
        end
        puts table
    end
    ...
    def self.falcon_9
        table = Terminal::Table.new :title => "#{Rocket.second.rocket_name}", :headings => ["Cost", "Height", "Diameter", "Mass"] do |t|
            t << [
                Rocket.second.cost, 
                "#{Rocket.second.height} ft.", 
                "#{Rocket.second.diameter} ft.", "#{Rocket.second.mass} lb."
            ]
        end
            table2 = Terminal::Table.new  do |t|
